...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  setCurrentLanguageSelection(idiomaSeleccionado);
});
...
function setCurrentLanguageSelection(current_lang) {
  ...
  for (var i = 0; i < gt_options.length; i++) {
    ...
    gt_option_a = gt_option.querySelectorAll("a");
    for (var j = 0; j < gt_option_a.length; j++) {
      gt_option_a[j].classList.remove("gt_current");
      if (gt_option_a[j].getAttribute("data-gt-lang") == current_lang) {
        gt_option_a[j].classList.add("gt_current");
      }
    }
  }
}
(function () {
  ...
  var gt_current_div =
    ...
    get_flag_src(localStorage.getItem("idioma")|| default_language) +
    '" height="' +
    flag_size +
    '" width="' +
    flag_size +
    ...
    current_lang +
    '" /> ' +
    lang_array[localStorage.getItem("idioma") || default_language] +
    "</a></div>";
